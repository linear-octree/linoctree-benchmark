...
void printHelp() {
	std::cout
		...
		<< "--config: Path to YAML configuration file (all options can be set in the file; CLI args override config file values)\n";
	exit(1);
}
	...
	while (true)
	{
		...
		switch (opt)
		{
			...
			case LongOptions::PCL_OCT_RESOLUTION:
				...
				break;
			case LongOptions::CONFIG_FILE:
				loadConfigFromYaml(std::filesystem::path(std::string(optarg)));
				break;
			default:
				printHelp();
				break;
